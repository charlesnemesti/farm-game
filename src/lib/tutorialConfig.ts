...
import { GAME_NAME, STORAGE_PREFIX } from "./brandConfig";
...
export const TUTORIAL_STORAGE_KEY = `${STORAGE_PREFIX}-tutorial-v1`;
...
const TUTORIAL_STEP_CONTENT: Omit<TutorialStepConfig, "id">[] = [
  {
    ...
    body: [
      "This guide walks through the game in order:",
      "",
      "• Core farming loop",
      "• Weather system",
      "• Wallet & treasury",
      "• Weekly leaderboard",
      "",
      "Interactive steps highlight what to click. Later steps explain the rest.",
    ].join("\n"),
  },
  {
    kind: "interactive",
  ...
  },
  {
    ...
    body: [
      `Purchase a Seeds Pack for ${SEED_PACK_ITEM.priceCorn.toLocaleString("en-US")} $CORN.`,
      "",
  ...
  },
  {
    ...
    body: [
      "Wait for the crop to finish growing.",
      "",
      "You earn $CORN and XP automatically when a cycle completes.",
    ].join("\n"),
    target: "furrow",
  },
  {
    ...
    body: [
      `Every ${weatherCycleMinutes} minutes the weather wheel spins and picks a random climate.`,
      "",
      "Chances:",
      `• Sunny — ${WEATHER_ROLL_WEIGHTS.sunny}%`,
      `• Rain — ${WEATHER_ROLL_WEIGHTS.rain}%`,
      `• Snow — ${WEATHER_ROLL_WEIGHTS.snow}%`,
      `• Wind — ${WEATHER_ROLL_WEIGHTS.wind}%`,
      "",
      "Effects:",
      `• Rain — +${rainBonusPct}% growth and $CORN`,
      `• Snow — −${snowPenaltyPct}% growth speed`,
      `• Wind — mature crops (${windTempestPct}%+ cycle) may uproot; seed is returned`,
      "",
      "Weather only applies while the tab is open. The countdown shows time until the next spin.",
    ].join("\n"),
  },
  {
    kind: "info",
    ...
    body: [
      `Crops keep growing for up to ${offlineCapHours} hours while you are away.`,
      "",
      "Weather modifiers do not apply offline — only standard growth rates.",
    ].join("\n"),
  },
  {
    kind: "info",
    ...
    body: [
      "Demo mode:",
      `• Browser saves only`,
      `• Row ${DEMO_MAX_PLOT_ID + 1} unlocked`,
      "",
      "Wallet mode:",
      "• Syncs to your address",
      "• Weekly leaderboard",
      "• Treasury deposit & withdraw",
      "",
      `Withdrawals unlock at level ${WITHDRAW_MIN_LEVEL}.`,
      "",
      "Deposit SPL $CORN to play. Withdraw earned $CORN on-chain when eligible.",
    ].join("\n"),
  },
  {
    kind: "info",
    ...
    body: [
      "Wallet players are ranked by $CORN/h production.",
      "",
      "Rankings reset every Monday 00:00 UTC.",
      "",
      "Open the game menu → Weekly rank for standings and treasury prizes.",
    ].join("\n"),
  },
  {
    kind: "complete",
    ...
    body: [
      "You know the loop and the systems around it.",
      "",
      "Keep planting, watch the weather, unlock rows, and grow your $CORN empire.",
      "",
      "Press Tutorial anytime at the bottom right to review this guide.",
    ].join("\n"),
  },
];
